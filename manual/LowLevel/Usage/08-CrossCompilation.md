...
> variable set by the Nix dev shell contains host-specific paths that conflict
> with the target. Clear it when cross-compiling.
...
A standard GHC installation cannot produce binaries for a different
architecture. GHC's base libraries contain architecture-specific code, the
RTS includes architecture-specific assembly, and GHC needs to know the
target's ABI (calling conventions, register usage, etc.).

A **cross-compiling GHC** runs on your host (e.g., x86_64) but produces
binaries for a target (e.g., aarch64), with target-specific libraries already
compiled.
...
| Method | Pros | Cons |
|--------|------|------|
| **Nix** (recommended) | Pre-built, reproducible, includes QEMU | Requires Nix |
| **System packages** | System integration | Rarely available |
| **Build from source** | Full control | Complex, hours to build |
