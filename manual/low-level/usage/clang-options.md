...
> the C compiler that GHC uses to compile the generated bindings.  In
> particular, a macro defined with `-D` via `--clang-option` or
> `BINDGEN_EXTRA_CLANG_ARGS` holds while the headers are parsed but not while
> the generated C wrapper source is compiled.  Use a `#define` root directive
> (`--hash-define NAME VALUE`, or `hashDefine` in Template Haskell) to define a
> macro for all C stages at once.  See [C stages][manual:c-stages].
