...
* __Generation stage__: `libclang` parses the headers you name, and `hs-bindgen`
  translates the declarations it finds into Haskell.

* __Compilation stage__: the generated Haskell modules embed C source, the
...
see [Non-portability][manual:non-portability] for what that costs.

## Root directives

`hs-bindgen` does not parse the headers you name one at a time.  It synthesises
a *root header* and parses that:
...
The lines of that header are the *root directives*: the `#include` directives
for the headers you name, and `#define` directives.  They are a single ordered
list.
...
`--hash-define` is position-sensitive, unlike Clang's `-D`, and why `hashDefine`
must precede the `hashInclude` calls it applies to.
...
| Clang `-D` argument | Command line               | Template Haskell          | Emitted directive  |
|---------------------|----------------------------|---------------------------|--------------------|
...
definition is reported by Clang as a diagnostic in the root header, the same
treatment an unresolvable `#include` gets.
...
The root header is `hs-bindgen`'s own, and declarations located in it are
skipped.  A `#define` stated as a root directive never produces a binding, under
any selection predicate, `--select-all` included.  It only changes how the
headers are preprocessed.  Define the macro in a header if you want a binding
for it.
...
(`extra-include-dirs`), and keeping those in agreement remains your job.  See
...
compilation stage.  A `-D` passed through one of those channels therefore
defines the macro for the generation stage alone; use a root directive instead.
See [Clang options][manual:clang-options].
