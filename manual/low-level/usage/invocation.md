...
The headers to translate are given as positional arguments.  They are
interleaved with `#define` root directives:
...
Use `hashDefine` for macros the headers expect.  It is `#define` syntax, not
Clang's `-D` syntax, and it only affects the `hashInclude`s that follow it:
...
The definition also reaches the generated C source that GHC compiles.  See
[C stages][manual:c-stages].
