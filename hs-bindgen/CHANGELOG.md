...
* Macro definitions are no longer a Clang argument but a *root directive*: an
  entry of the same ordered list as the `#include`s. The root header is rendered
  from that list, and the very same rendering is prepended to the generated CAPI
  wrapper source, so both C stages agree by construction. Previously a `-D FOO`
  had to be stated twice — once for `hs-bindgen` and once as
  `ghc-options: -optc-DFOO` in the `.cabal` file — with nothing keeping the two
  in agreement. See [issue #2214][is-2214] and the
  [C stages](../manual/low-level/usage/c-stages.md) manual page. Concretely:
    * `ClangArgsConfig.defineMacros` is **removed**. Use a `#define` root
      directive instead. Raw `-D` passed via `argsBefore`/`argsInner`/`argsAfter`
      or `BINDGEN_EXTRA_CLANG_ARGS` still reaches the generation stage only.
    ...
    * Behaviour: the generated C wrapper source now starts with the whole root
      ...
      wrapper translation unit includes every main header. A module with no
      wrappers now emits no C source at all.
    * Declarations in the root header (i.e. the `#define`s) are never selected,
      not even under `--select-all`: the root header is synthetic and is not a
      main header of anything.
