...
-- type t'CtxDoc' threads a t'Context'.  Keeping track of the current indentation
...
-- | Construct an initial t'Context' with the specified line length
...
-- | Add to the indentation in a t'Context'
...
-- | Run a t'CtxDoc' with the specified context
...
-- | Select a t'CtxDoc' depending on if a rendered t'CtxDoc' fits within the
-- maximum line length
