...
reverse DynGraph{vtxMap, idxMap, edges} = DynGraph{
  ...
  where
    go :: IntMap (Set (l, Int)) -> [(Int, Set (l, Int))] -> IntMap (Set (l, Int))
    go !acc []            = acc
    go !acc ((fr, to):es) = go (foldr (addEdge fr) acc to) es

    addEdge :: Int -> (l, Int) -> IntMap (Set (l, Int)) -> IntMap (Set (l, Int))
    addEdge fr (l, to) = IntMap.insertWith (<>) to $ Set.singleton (l, fr)
...
insertEdge vFrom l vTo dynGraph0 =
    ...
    in  dynGraph2 {
            edges =
              IntMap.insertWith (<>) vFromIdx (Set.singleton (l, vToIdx)) $
                edges dynGraph2
          }
...
findTrailFrom :: forall m l a r.
     (Monad m, Ord a)
  => DynGraph l a
  -> ([(l, a)] -> m (Either a r)) -- ^ Choose next step
  -> a                            -- ^ Starting point
  -> m r
    ...
    go curr = do
      ...
      where
        ...
        successors = fromMaybe [] $ do
            ...
            return $ map (second (idxMap IntMap.!)) next
...
-- same time.
reaches' :: Ord l => IntMap (Set (l, Int)) -> Int -> [Int]
  ...
  where
    ...
    aux acc (x:xs) =
      case Map.insertLookupWithKey (\_key _new old -> old) x () acc of
        (Just (), _)    -> aux acc xs
        (Nothing, acc') -> aux acc' $
          map snd (Set.toList (IntMap.findWithDefault mempty x edgeMap)) ++ xs
          ...
          aux (idx:idxs) = do
            ...
            if visited
              then aux idxs
                ...
                include idx
                children <-
                  aux . map snd $ maybe [] Set.toList (IntMap.lookup idx edges)
                trees <- aux idxs
