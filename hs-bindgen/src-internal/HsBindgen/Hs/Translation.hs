...
generateDeclarations opts _mu nm (C.Header decs) =
    flip State.evalState Map.empty $
      concat <$> mapM (generateDecs opts nm nameMap typedefs) decs
  where
  ...
  where
    ...
    aux acc (hsType:hsTypes)
      ...
      | otherwise = case hsType of
          ...
          HsIO{} -> Set.empty
          HsFun{} -> Set.empty
          HsExtBinding extType _cType ->
            let acc' = acc /\ BindingSpecs.extTypeInstances extType
            in  aux acc' hsTypes
          HsByteArray{} ->
...
generateDecs ::
  ...
  -> C.Decl
  -> m [Hs.Decl]
generateDecs opts nm nameMap typedefs = \case
    C.DeclStruct struct  ->
    ...
    C.DeclFunction f     -> return $ functionDecs nm nameMap typedefs f
...
functionDecs ::
  ...
  -> C.Function
  -> [Hs.Decl]
functionDecs nm nameMap typedefs f
