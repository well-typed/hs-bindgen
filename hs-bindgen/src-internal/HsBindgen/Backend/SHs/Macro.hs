...
integerLiteral lit =
    EIntegral (DSL.integerLiteralValue lit) $
      Just $ bindgenGlobal $ runtimeIntegral $ Runtime.IntLike (DSL.integerLiteralType lit)
...
floatingLiteral lit =
    case DSL.floatingLiteralType lit of
      Runtime.FloatType ->
        EFloat  (DSL.floatingLiteralFloatValue  lit) (bindgenGlobal CFloat_type)
      Runtime.DoubleType ->
        EDouble (DSL.floatingLiteralDoubleValue lit) (bindgenGlobal CDouble_type)

{-------------------------------------------------------------------------------
  Primitive types
-------------------------------------------------------------------------------}

dslIntegral :: DSL.IntegralType -> BindgenGlobal
dslIntegral = \case
    DSL.CIntegralType primIntTy -> runtimeIntegral primIntTy
    DSL.HsIntType               -> Int_type

runtimeIntegral :: Runtime.IntegralType -> BindgenGlobal
runtimeIntegral = \case
    Runtime.Bool       -> CBool_type
    Runtime.CharLike c -> runtimeCharLike c
    Runtime.IntLike i  -> runtimeIntLike i

runtimeCharLike :: Runtime.CharLikeType -> BindgenGlobal
runtimeCharLike = \case
    Runtime.Char  -> CChar_type
    Runtime.SChar -> CSChar_type
    Runtime.UChar -> CUChar_type

runtimeIntLike :: Runtime.IntLikeType -> BindgenGlobal
runtimeIntLike = \case
...
runtimeFloating :: Runtime.FloatingType -> BindgenGlobal
...
tyCon (DSL.GenerativeTyCon (DSL.ClassTyCon tc)) = TGlobal $ cExprGlobal $ classTyCon  tc
tyCon (DSL.FamilyTyCon tc)                      = TGlobal $ cExprGlobal $ familyTyCon tc

dataTyCon :: DSL.DataTyCon n -> SType ctx
dataTyCon = \case
    DSL.TupleTyCon n          -> TTup $ fromIntegral n
    ...
    DSL.PtrTyCon              -> tBindgenGlobal   Foreign_Ptr
    DSL.CharLitTyCon          -> tBindgenGlobal   CharValue_tycon
...
classTyCon :: DSL.ClassTyCon args -> CExprGlobal
classTyCon = \case
...
familyTyCon :: DSL.FamilyTyCon args -> CExprGlobal
familyTyCon = \case
...
mfun = \case
    DSL.MUnaryPlus  -> EGlobal $ cExprGlobal Plus_plus
    DSL.MUnaryMinus -> EGlobal $ cExprGlobal Minus_negate
    DSL.MLogicalNot -> EGlobal $ cExprGlobal Not_not
    DSL.MBitwiseNot -> EGlobal $ cExprGlobal Complement_complement
    DSL.MMult       -> EGlobal $ cExprGlobal Mult_mult
    DSL.MDiv        -> EGlobal $ cExprGlobal Div_div
    DSL.MRem        -> EGlobal $ cExprGlobal Rem_rem
    DSL.MAdd        -> EGlobal $ cExprGlobal Add_add
    DSL.MSub        -> EGlobal $ cExprGlobal Sub_minus
    DSL.MShiftLeft  -> EGlobal $ cExprGlobal Shift_shiftL
    DSL.MShiftRight -> EGlobal $ cExprGlobal Shift_shiftR
    DSL.MRelLT      -> EGlobal $ cExprGlobal RelOrd_lt
    DSL.MRelLE      -> EGlobal $ cExprGlobal RelOrd_le
    DSL.MRelGT      -> EGlobal $ cExprGlobal RelOrd_gt
    DSL.MRelGE      -> EGlobal $ cExprGlobal RelOrd_ge
    DSL.MRelEQ      -> EGlobal $ cExprGlobal RelEq_eq
    DSL.MRelNE      -> EGlobal $ cExprGlobal RelEq_uneq
    DSL.MBitwiseAnd -> EGlobal $ cExprGlobal Bitwise_and
    DSL.MBitwiseXor -> EGlobal $ cExprGlobal Bitwise_xor
    DSL.MBitwiseOr  -> EGlobal $ cExprGlobal Bitwise_or
    DSL.MLogicalAnd -> EGlobal $ cExprGlobal Logical_and
    DSL.MLogicalOr  -> EGlobal $ cExprGlobal Logical_or
    DSL.MTuple @n   -> ETup $ 2 + Fin.reflectToNum @n Proxy
