...
-- > (# state, EmptyStruct #)
buildNestedReads :: forall n ctx. Hs.Struct n -> BindgenGlobal -> Idx ctx -> Idx ctx -> Idx ctx -> [(HsType, Int)] -> [Idx ctx] -> Int -> SExpr ctx
...
-- For an empty struct (no fields), generates:
-- > state
buildSequentialWrites :: forall ctx. BindgenGlobal -> Idx ctx -> Idx ctx -> Idx ctx -> [(HsType, Int, Idx ctx)] -> Int -> SExpr ctx
