...
appMany :: BindgenGlobal -> [SExpr ctx] -> SExpr ctx
...
idiom f g (Hs.Ap p xs) = foldl'
    (\ acc x -> EInfix (bindgenGlobal Applicative_seq) acc (g x))
...
doAll _ (Hs.Seq []) = eBindgenGlobal Monad_return `EApp` ETup 0
doAll f (Hs.Seq ss) = foldr1 (EInfix (bindgenGlobal Monad_seq)) (map f ss)
