...
generateDeclarations macroLang uniqueId name declIndex =
    fmap reverse .
  ...
  where
    ...
    env :: HsM.Env
    env = HsM.initEnv uniqueId name supInsts
  ...
  where
    ...
    aux env =
        let
          ...
          patSynType :: Hs.Type
          patSynType = Hs.PrimType (translateCPrimType enumConstant.typ)
