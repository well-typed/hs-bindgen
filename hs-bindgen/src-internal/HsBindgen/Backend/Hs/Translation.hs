...
hsWrapperDecl
    ...
    -> [WrappedType]  -- ^ arguments
    -> SHs.SDecl
hsWrapperDecl typedefs hiName loName res args = case res of
  HeapType {} ->
    SHs.DVar
      SHs.Var {
        varName    = hiName
...
functionDecs safety opts haddockConfig moduleName typedefs info f _spec =
    funDecl : [
        Hs.DeclSimple $ hsWrapperDecl typedefs highlevelName importName res wrappedArgTypes
      | anyFancy
      ]
  where
    funDecl :: Hs.Decl
    ...
    wrapType ty = go ty
      where
        go = \case
          ...
          (C.TypeTypedef (C.Full ref)) -> go $ C.eraseTypedef typedefs ref
          _                            -> WrapType ty
