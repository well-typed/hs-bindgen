  ...
  where
    ...
    isStructDecl = [
          Hs.DeclDeriveInstance Hs.DeriveInstance{
              strategy = Hs.DeriveVia (Hs.IsStructViaReadRaw (Hs.TypRef hsStruct.name Nothing))
            , clss     = Inst.IsStruct
            , name     = hsStruct.name
            , comment  = Nothing
            }
    ...
    knownInsts = Set.fromList $ catMaybes [
      ...
      , if Inst.ReadRaw `elem` insts && Inst.StaticSize `elem` insts
          then Just Inst.IsStruct else Nothing
      ]
