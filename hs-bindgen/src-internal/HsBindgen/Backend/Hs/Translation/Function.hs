...
--   function.
--
-- TODO_PR: Fancy -> non-primtive signature/primitive signature.
functionDecs ::
     HasCallStack
  => SHs.Safety
  -> TranslationConfig
...
functionDecs safety opts haddockConfig moduleName info origCFun _spec =
  ...
  where
    ...
    isFancyOrigCFun :: Bool
    isFancyOrigCFun = any isFancy (fancyResult : fancyParams)

    fancyResult :: IsPrimitiveType
    fancyResult = toIsPrimitiveType $ C.functionRes origCFun

    fancyParams :: [IsPrimitiveType]
    fancyParams = map (toIsPrimitiveType . snd) origCFun.functionArgs
    ...
    foreignImportParams = [
           ...
           , functionParameterType    = Type.inContext Type.FunArg (toLowlevelType (toIsPrimitiveType ty))
    ...
    (mbAliasComment, aliasParams) =
      generateHaddocksDecorateParams haddockConfig info foreignImportParams
    ...
    restoreOrigSignatureParams' = [
           Hs.FunctionParameter
           { functionParameterName    = fmap C.nameHs mbName
           , functionParameterType    = Type.inContext Type.FunArg (toHighlevelType (toIsPrimitiveType ty))
           , functionParameterComment = Nothing
           }
        | (mbName, ty) <- C.functionArgs origCFun
        ]
    ...
    (mbRestoreOrigSignatureComment, restoreOrigSignatureParams) =
      generateHaddocksDecorateParams haddockConfig info restoreOrigSignatureParams'
    ...
    (mbResultParam, resultType) = case fancyResult of
        ...
        HeapType {} -> (Just Hs.FunctionParameter {
          ...
          , functionParameterType = Type.inContext Type.FunArg $ toLowlevelType fancyResult
          , functionParameterComment = Nothing
          }
        ...
        NonFancyType {} ->
          ( Nothing
          , mbIO $ Type.inContext Type.FunRes $ toLowlevelType fancyResult
          )
...
-- | Heap types and constant arrays are fancy types. We have to treat functions
--   with fancy types in a special way.
isFancy :: IsPrimitiveType -> Bool
isFancy = \case
    NonFancyType{} -> False
    HeapType{} -> True
    CAType{}   -> True
...
toIsPrimitiveType ty
  ...
  = if C.isErasedTypeConstQualified ty then
      ...
      NonFancyType $ C.TypePointer (C.getArrayElementType aTy)
...
-- TODO_PR: rename to 'toPrimitiveType'.
toLowlevelType :: IsPrimitiveType -> C.Type
toLowlevelType = \case
    NonFancyType ty -> ty
...
-- TODO_PR: 'toOrigType'
toHighlevelType :: IsPrimitiveType -> C.Type
toHighlevelType (NonFancyType ty) = ty
toHighlevelType (HeapType ty)     = ty
toHighlevelType (CAType oty _ _)  = oty
toHighlevelType (AType oty _)     = oty
...
getCWrapperDecl origName wrapperName res args
    ...
    = PC.withArgs args $ \args' ->
        PC.FunDefn wrapperName C.TypeVoid C.ImpureFunction (toLowlevelType <$> args')
          [PC.Expr $ PC.Call origName (callArgs args' (PC.argsToIdx args'))]

    | isHeapType res
    = PC.withArgs args $ \args' ->
        PC.FunDefn wrapperName C.TypeVoid C.ImpureFunction (toLowlevelType <$> (args' :> res))
          [PC.Assign (PC.LDeRef (PC.LVar IZ)) $ PC.Call origName (callArgs args' (IS <$> PC.argsToIdx args'))]

    | otherwise
    = PC.withArgs args $ \args' ->
        PC.FunDefn wrapperName (toLowlevelType res) C.ImpureFunction (toLowlevelType <$> args')
...
getRestoreOrigSignatureDecl ::
  ...
  -> Hs.Decl
getRestoreOrigSignatureDecl hiName loName fancyResult fancyParams params cFunc mbComment =
    let resType :: HsType
        resType = Type.inContext Type.FunRes $ toHighlevelType fancyResult
    in  case fancyResult of
      HeapType {} ->
        Hs.DeclFunction $ Hs.FunctionDecl
          { functionDeclName       = hiName
          , functionDeclParameters = params
          , functionDeclResultType = HsIO resType
          , functionDeclBody       = goA EmptyEnv fancyParams
      ...
      NonFancyType {} ->
        Hs.DeclFunction $ Hs.FunctionDecl
          ...
          , functionDeclParameters = params
          , functionDeclResultType = HsIO resType
          , functionDeclBody       = goB EmptyEnv fancyParams
          , functionDeclOrigin     = Origin.Function cFunc
          , functionDeclComment    = mbComment
          }
