...
annSortKey order decl = SortKey{
  ...
  where
    sortPathIx = case IncludeGraph.lookupIncludeOrder order (singleLocPath decl.info.loc) of
      IncludeGraph.NotInIncludeGraph ->
        panicPure $ "Source of declaration " <> show (singleLocPath decl.info.loc) <> " not in include graph"
      ix -> ix
