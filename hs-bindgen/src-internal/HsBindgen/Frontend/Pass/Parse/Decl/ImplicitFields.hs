...
withImplicitFields encObj inputs = do
  ...
  where
    ...
    getImplicitField' ::
      ...
      -> M m (Numbered (C.ImplicitField Parse))
...
getImplicitField ::
  ...
  -> C.Decl l Parse
  -> M m (C.ImplicitField Parse)
getImplicitField encObj decl = do
    candFields <- getCandidateFields encObj decl
    candFieldsNE <- liftEither $ checkNonEmpty candFields
    -- We do not assume that the fields are ordered in any way, hence we use
    -- 'minimumBy' on the raw offset to determine which candidate field is the
...
getCandidateFields ::
  ...
  => EnclosingObject
     -- | An anonymous object nested in the enclosing object
  -> C.Decl l Parse
  -> M m [CandidateField Parse]
getCandidateFields encObj decl = forM fields $ \field ->
    getCandidateField encObj decl field
  where
    fields :: [C.Field Parse]
...
getCandidateField ::
  ...
  -> C.Field Parse
  -> M m (CandidateField Parse)
getCandidateField encObj decl field = do
    offsetOuter <- offsetOf encObj origin
  ...
  where
    ...
    -- field names.
    origin = C.elimField (.info.name) (.ann.field) field

    mkCandidateField :: Int -> CandidateField Parse
    mkCandidateField offset = CandidateField {
          info = C.FieldInfo {
                loc = decl.info.loc
              , name = C.ScopedName ("anon'" <> field.info.name.text)
              , comment = ()
              }
        , typRef = C.AnonRef decl.info.id
          -- The offset to the implicit field is equal to the offset to any of
          -- the nested object's fields with respect to the enclosing object,
          -- subtracted by the offset of that same field with respect to the
          -- nested object.
        , offset = offset - field.offset
        , ann = Origin.FieldOrigin origin
        , rawOffset = offset
        }

-- | A candidate implicit field
data CandidateField p = CandidateField {
      info   :: C.FieldInfo p
    , typRef :: C.AnonRef p
    , offset :: Int
    , ann    :: Origin.FieldOrigin
