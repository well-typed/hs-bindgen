...
macroDefinition macroLang _enclosing ctx info = \curr -> do
  ...
  where
    ...
    -- Whether an empty macro body (@#define FOO@) can be translated is the
    -- language's decision: 'Raw' translates it, @CExpr@ has no expression to
    -- translate. A language that declines is not a failure worth reporting,
    -- though, since every include guard takes this path.
    macroParseMsg ::
         Either MacroParseError (RawMacro.Raw (Token TokenSpelling))
      -> MacroParseError
      -> DelayedParseMsg
