...
instance PrettyForTrace SelectMsg where
  prettyForTrace = \case
    ...
    SelectMissedParseMsg  x -> "Missed declaration: " >< prettyForTrace x
...
instance IsTrace Level SelectMsg where
  getDefaultLogLevel = \case
    ...
    SelectExcluded{}                        -> Info
    SelectSelected{}                        -> Info
    SelectMissedParseMsg  x                 -> getDefaultLogLevel x
  getSource  = const HsBindgen
  getTraceId = \case
    SelectMissedParseMsg  x -> "select-miss-" <> getTraceId x
    _else                   -> "select"
