...
instance IsPass Select where
  ...
  type TypedefRef   Select = TypedefRefRegular Select
...
instance CoercePass TypedefRefWrapper ResolveBindingSpec Select where
  coercePass (TypedefRefWrapper (TypedefRefRegular n uTy)) =
      TypedefRefWrapper $ TypedefRefRegular n (coercePass uTy)
