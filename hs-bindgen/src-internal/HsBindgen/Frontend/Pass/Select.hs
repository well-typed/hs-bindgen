    ...
    case selectConfigProgramSlicing of
      DisableProgramSlicing ->
        let matchedDecls :: [C.Decl Select]
            ...
            parseMsgs :: [Msg Parse]
            missedParseMsgs :: [Msg Select]
            (parseMsgs, missedParseMsgs) = getParseMsgs' matchedDecls

            selectMsgs :: [Msg Select]
            selectMsgs = map (SelectSelected . C.declInfo) matchedDecls
         in ( unitSelect { C.unitDecls = matchedDecls }
            , parseMsgs, missedParseMsgs ++ selectMsgs
            )
      ...
      EnableProgramSlicing ->
        let matchedDecls, unmatchedDecls :: [C.Decl Select]
            ...
            parseMsgs :: [Msg Parse]
            missedParseMsgs :: [Msg Select]
            (parseMsgs, missedParseMsgs) = getParseMsgs' selectedDecls
...
getParseMsgs meta match decls =
     (concat selectedMsgs, map SelectMissedParseMsg (concat missedMsgs))
  where
    ...
    -- Missed declarations are declarations we desired to select, but that were
    -- skipped during parse/reification. See the documentation of 'ParseMsgs'.
    missedKeys :: Set Key
    missedKeys = desiredSelectedKeys Set.\\ selectedKeys

    selectedMsgs :: Map Key [ParseMsg]
    selectedMsgs = Map.restrictKeys msgs selectedKeys

    missedMsgs :: Map Key [ParseMsg]
    missedMsgs = Map.restrictKeys msgs missedKeys
