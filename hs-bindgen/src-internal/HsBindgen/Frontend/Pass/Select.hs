...
selectDecls isMainHeader SelectConfig{..} unitRBS =
    case selectConfigProgramSlicing of
      DisableProgramSlicing -> (unitSelect, [])
      ...
      EnableProgramSlicing ->
        let useDeclGraph :: UseDeclGraph
            ...
            matchDecl decl =
                Predicate.matchSelect isMainHeader loc qid selectConfigPredicate
              where
                loc :: SingleLoc
                loc = C.declLoc $ C.declInfo decl

                qid :: C.QualDeclId
                qid = C.declQualDeclId decl

            matchedDeclarations   :: [C.Decl Select]
