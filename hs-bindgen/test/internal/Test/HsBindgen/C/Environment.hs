module Test.HsBindgen.C.Environment (tests) where
...
import HsBindgen.Clang.Args (getExtraClangArgs, splitArguments)
import HsBindgen.Lib (Target (Target_Linux_X86_64))
...
tests :: TestTree
tests = testGroup "HsBindgen.C.Environment" [envTests, splitArgumentsTests]
...
envTests :: TestTree
envTests = testGroup "getExtraClangArgs" [
          testCase "!target" $
            assertWithEnv [(eDef, "native")]
              (getExtraClangArgs Nothing) ["native"]
        ...
        , testCase "target" $
            assertWithEnv [(eLnx, "cross")]
              (getExtraClangArgs (Just Target_Linux_X86_64)) ["cross"]
        ...
        , -- With target, we fall back to the default `clang` arguments.
          testCase "target+!other" $
            assertWithEnv [(eDef, "native")]
              (getExtraClangArgs (Just Target_Linux_X86_64)) ["native"]
