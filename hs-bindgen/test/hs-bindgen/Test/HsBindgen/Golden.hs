...
testCases = [
    ...
    , testTraceCustom "decls_in_signature" ["f3", "f4", "f5"] $ \case
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnexpectedAnonInSignature info))) ->
    ...
    , testTraceCustom "redeclaration" ["x", "n"] $ \case
        TraceFrontend (FrontendParse (ParsePotentialDuplicateSymbol info _isPublic)) ->
          Just $ Expected (C.declId info)
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnknownStorageClass info (unsafeFromSimpleEnum -> CX_SC_Static)))) ->
          Just $ Expected (C.declId info)
        _otherwise ->
          Nothing
    , testTraceCustom "skip_over_long_double" ["fun1", "struct1"] $ \case
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnsupportedType info UnsupportedLongDouble))) ->
          Just $ Expected $ C.declId info
        _otherwise ->
          Nothing
    , testTraceCustom "tentative_definitions" ["i1", "i2", "i3", "i3"] $ \case
        TraceFrontend (FrontendParse (ParsePotentialDuplicateSymbol info _isPublic)) ->
          Just $ Expected (C.declId info)
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParsePotentialDuplicateSymbol info _isPublic))) ->
          Just $ Expected (C.declId info)
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnknownStorageClass info (unsafeFromSimpleEnum -> CX_SC_Static)))) ->
          Just $ Expected (C.declId info)
    ...
    , testTraceCustom "varargs" ["f", "g"] $ \case
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnsupportedType info UnsupportedVariadicFunction))) ->
          Just $ Expected $ C.declId info
        _otherwise ->
          Nothing
    ...
    , failingTestSimple "long_double" $ \case
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnsupportedType _ UnsupportedLongDouble))) ->
          Just $ Expected ()
        _otherwise ->
          Nothing
    , failingTestSimple "implicit_fields_struct" $ \case
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnsupportedImplicitFields {}))) ->
          Just $ Expected ()
        _otherwise ->
          Nothing
    ...
    , failingTestSimple "unsupported_builtin" $ \case
        TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnsupportedType _info (UnsupportedBuiltin "__builtin_va_list")))) ->
          Just $ Expected ()
        _otherwise ->
          Nothing
    ...
    , let declsWithWarnings :: [C.PrelimDeclId]
      ...
      in (defaultTest "globals") {
        ...
        , testTracePredicate = customTracePredicate' declsWithWarnings $ \case
            TraceFrontend (FrontendParse (ParsePotentialDuplicateSymbol info _isPublic)) ->
              Just $ Expected (C.declId info)
            TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnexpectedAnonInExtern info))) ->
              Just $ Expected (C.declId info)
            _otherwise ->
              Nothing
        }
    ...
    , (defaultFailingTest "thread_local"){
        ...
        , testTracePredicate = singleTracePredicate $ \case
            TraceFrontend (FrontendSelect (SelectMissedParseMsg (ParseUnsupportedTLS{}))) ->
              Just $ Expected ()
            _otherwise ->
              Nothing
        }
