...
instance Arbitrary TagName where
  arbitrary = TagName . NE.toList . (.unwrap) <$> liftArbitrary @MyNonEmpty f
    where
      f = elements $ concat [
            [ 'a' .. 'z' ]
          , [ 'A' .. 'Z' ]
...
-- 'NonEmpty' wrapper for compatibility with QuickCheck
--
-- QuickCheck implements 'Arbitrary' instances for 'NonEmpty' only since version
-- 2.17
newtype MyNonEmpty a = MyNonEmpty { unwrap :: NonEmpty a }

instance Arbitrary a => Arbitrary (MyNonEmpty a) where
  arbitrary = liftArbitrary arbitrary
  shrink = liftShrink shrink

instance Arbitrary1 MyNonEmpty where
  liftArbitrary f = MyNonEmpty <$> ((:|) <$> f <*> listOf f)
  liftShrink f (MyNonEmpty (x :| xs)) = [
        MyNonEmpty (NE.fromList xs')
