...
foreign import ccall safe "test_internal_quux" quux :: F -> FC.CChar -> IO FC.CChar

foreign import ccall safe "test_internal_wam" wam :: FC.CFloat -> (F.Ptr C) -> IO (F.Ptr C)

foreign import ccall safe "test_internal_foo1" foo1 :: FC.CFloat -> (F.FunPtr (FC.CInt -> IO FC.CInt)) -> IO (F.Ptr FC.CChar)

foreign import ccall safe "test_internal_foo2" foo2 :: F -> (F.FunPtr (FC.CInt -> IO FC.CInt)) -> IO (F.Ptr FC.CChar)

foreign import ccall safe "test_internal_foo3" foo3 :: FC.CFloat -> (F.FunPtr (FC.CInt -> IO FC.CInt)) -> IO (F.Ptr C)

foreign import ccall safe "test_internal_bar1" bar1 :: FC.CLong -> IO (F.FunPtr (FC.CShort -> IO FC.CInt))

foreign import ccall safe "test_internal_bar2" bar2 :: L -> IO (F.FunPtr (FC.CShort -> IO FC.CInt))

foreign import ccall safe "test_internal_bar3" bar3 :: FC.CLong -> IO (F.FunPtr (S -> IO FC.CInt))

foreign import ccall safe "test_internal_bar4" bar4 :: FC.CLong -> IO (F.FunPtr (FC.CShort -> IO I))
...
foreign import ccall safe "test_internal_baz2" baz2 :: I -> IO (F.Ptr ((HsBindgen.Runtime.ConstantArray.ConstantArray 2) ((HsBindgen.Runtime.ConstantArray.ConstantArray 3) FC.CInt)))

foreign import ccall safe "test_internal_baz3" baz3 :: FC.CInt -> IO (F.Ptr ((HsBindgen.Runtime.ConstantArray.ConstantArray 2) ((HsBindgen.Runtime.ConstantArray.ConstantArray 3) I)))
