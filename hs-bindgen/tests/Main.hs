...
main = do
    ...
    defaultMain $ testGroup "hs-bindgen-tests"
      [ unitTests packageRoot
      , withRustBindgen $ goldenTests packageRoot
      ]
...
goldenTests :: FilePath -> IO FilePath -> TestTree
goldenTests packageRoot rustBindgen = testGroup "golden"
  ...
  where
    ...
    golden name = testGroup name $
      goldenNoRust name ++ [goldenRust rustBindgen name]
