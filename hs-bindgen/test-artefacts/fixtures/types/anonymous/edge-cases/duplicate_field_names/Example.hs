...
deriving via Struct.IsStructViaStorable S_y_anon'x instance Struct.IsStruct S_y_anon'x
...
deriving via Struct.IsStructViaStorable S_y instance Struct.IsStruct S_y
      ...
      \s1 ->
        case s1 of
          S s_x2 s_y3 ->
            ...
            >> HasCField.writeRaw (BG.Proxy @"s_y") ptr0 s_y3
...
instance (ty ~ BG.CULLong) => BG.CompatHasField.HasField "s_x" S ty where
  ...
  hasField =
    \x0 ->
      (\y1 ->
         S {s_x = y1, s_y = BG.getField @"s_y" x0}, BG.getField @"s_x" x0)

instance (ty ~ BG.CULLong) => BG.HasField "s_x" (BG.Ptr S) (BG.Ptr ty) where

  getField = HasCField.fromPtr (BG.Proxy @"s_x")

instance HasCField.HasCField S "s_x" where

  type CFieldType S "s_x" = BG.CULLong
...
instance (ty ~ S_y) => BG.CompatHasField.HasField "s_y" S ty where
  ...
  hasField =
    \x0 ->
      (\y1 ->
         S {s_y = y1, s_x = BG.getField @"s_x" x0}, BG.getField @"s_y" x0)
