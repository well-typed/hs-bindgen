{-# LANGUAGE DeriveGeneric #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE DerivingVia #-}
{-# LANGUAGE StandaloneDeriving #-}

module Example
    ( Example.A(..)
    , Example.Struct1(..)
    , Example.B_s(..)
    , Example.Struct2_s(..)
    , Example.C(..)
    , Example.Struct3(..)
    , Example.D(..)
    , Example.Struct4(..)
    , Example.E_s(..)
    , Example.Struct5_s(..)
    , Example.F(..)
    , Example.Typedef1(..)
    , Example.G(..)
    , Example.Typedef2(..)
    , Example.H(..)
    , Example.Typedef3(..)
    )
  where

import qualified HsBindgen.Runtime.Marshal as Marshal
import qualified HsBindgen.Runtime.Struct as Struct
import qualified HsBindgen.Runtime.Support as BG

{-| __C declaration:__ @struct a@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 15:8@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data A = A
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize A where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw A where

  readRaw = \ptr0 -> pure A

instance Marshal.WriteRaw A where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          A -> return ()

deriving via Marshal.EquivStorable A instance BG.Storable A

deriving via Struct.IsStructViaReadRaw A instance Struct.IsStruct A

{-| __C declaration:__ @struct struct1@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 16:8@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Struct1 = Struct1
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Struct1 where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Struct1 where

  readRaw = \ptr0 -> pure Struct1

instance Marshal.WriteRaw Struct1 where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Struct1 -> return ()

deriving via Marshal.EquivStorable Struct1 instance BG.Storable Struct1

deriving via Struct.IsStructViaReadRaw Struct1 instance Struct.IsStruct Struct1

{-| __C declaration:__ @struct b_s@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 19:8@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data B_s = B_s
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize B_s where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw B_s where

  readRaw = \ptr0 -> pure B_s

instance Marshal.WriteRaw B_s where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          B_s -> return ()

deriving via Marshal.EquivStorable B_s instance BG.Storable B_s

deriving via Struct.IsStructViaReadRaw B_s instance Struct.IsStruct B_s

{-| __C declaration:__ @struct struct2_s@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 20:8@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Struct2_s = Struct2_s
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Struct2_s where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Struct2_s where

  readRaw = \ptr0 -> pure Struct2_s

instance Marshal.WriteRaw Struct2_s where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Struct2_s -> return ()

deriving via Marshal.EquivStorable Struct2_s instance BG.Storable Struct2_s

deriving via Struct.IsStructViaReadRaw Struct2_s instance Struct.IsStruct Struct2_s

{-| __C declaration:__ @struct c@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 23:36@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data C = C
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize C where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw C where

  readRaw = \ptr0 -> pure C

instance Marshal.WriteRaw C where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          C -> return ()

deriving via Marshal.EquivStorable C instance BG.Storable C

deriving via Struct.IsStructViaReadRaw C instance Struct.IsStruct C

{-| __C declaration:__ @struct struct3@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 24:36@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Struct3 = Struct3
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Struct3 where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Struct3 where

  readRaw = \ptr0 -> pure Struct3

instance Marshal.WriteRaw Struct3 where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Struct3 -> return ()

deriving via Marshal.EquivStorable Struct3 instance BG.Storable Struct3

deriving via Struct.IsStructViaReadRaw Struct3 instance Struct.IsStruct Struct3

{-| __C declaration:__ @struct d@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 27:15@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data D = D
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize D where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw D where

  readRaw = \ptr0 -> pure D

instance Marshal.WriteRaw D where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          D -> return ()

deriving via Marshal.EquivStorable D instance BG.Storable D

deriving via Struct.IsStructViaReadRaw D instance Struct.IsStruct D

{-| __C declaration:__ @struct struct4@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 28:15@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Struct4 = Struct4
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Struct4 where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Struct4 where

  readRaw = \ptr0 -> pure Struct4

instance Marshal.WriteRaw Struct4 where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Struct4 -> return ()

deriving via Marshal.EquivStorable Struct4 instance BG.Storable Struct4

deriving via Struct.IsStructViaReadRaw Struct4 instance Struct.IsStruct Struct4

{-| __C declaration:__ @struct e_s@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 31:15@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data E_s = E_s
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize E_s where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw E_s where

  readRaw = \ptr0 -> pure E_s

instance Marshal.WriteRaw E_s where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          E_s -> return ()

deriving via Marshal.EquivStorable E_s instance BG.Storable E_s

deriving via Struct.IsStructViaReadRaw E_s instance Struct.IsStruct E_s

{-| __C declaration:__ @struct struct5_s@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 32:15@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Struct5_s = Struct5_s
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Struct5_s where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Struct5_s where

  readRaw = \ptr0 -> pure Struct5_s

instance Marshal.WriteRaw Struct5_s where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Struct5_s -> return ()

deriving via Marshal.EquivStorable Struct5_s instance BG.Storable Struct5_s

deriving via Struct.IsStructViaReadRaw Struct5_s instance Struct.IsStruct Struct5_s

{-| __C declaration:__ @struct f@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 35:9@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data F = F
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize F where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw F where

  readRaw = \ptr0 -> pure F

instance Marshal.WriteRaw F where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          F -> return ()

deriving via Marshal.EquivStorable F instance BG.Storable F

deriving via Struct.IsStructViaReadRaw F instance Struct.IsStruct F

{-| __C declaration:__ @struct typedef1@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 36:9@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Typedef1 = Typedef1
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Typedef1 where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Typedef1 where

  readRaw = \ptr0 -> pure Typedef1

instance Marshal.WriteRaw Typedef1 where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Typedef1 -> return ()

deriving via Marshal.EquivStorable Typedef1 instance BG.Storable Typedef1

deriving via Struct.IsStructViaReadRaw Typedef1 instance Struct.IsStruct Typedef1

{-| __C declaration:__ @struct g@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 39:9@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data G = G
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize G where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw G where

  readRaw = \ptr0 -> pure G

instance Marshal.WriteRaw G where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          G -> return ()

deriving via Marshal.EquivStorable G instance BG.Storable G

deriving via Struct.IsStructViaReadRaw G instance Struct.IsStruct G

{-| __C declaration:__ @struct typedef2@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 40:9@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Typedef2 = Typedef2
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Typedef2 where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Typedef2 where

  readRaw = \ptr0 -> pure Typedef2

instance Marshal.WriteRaw Typedef2 where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Typedef2 -> return ()

deriving via Marshal.EquivStorable Typedef2 instance BG.Storable Typedef2

deriving via Struct.IsStructViaReadRaw Typedef2 instance Struct.IsStruct Typedef2

{-| __C declaration:__ @struct h@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 43:9@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data H = H
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize H where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw H where

  readRaw = \ptr0 -> pure H

instance Marshal.WriteRaw H where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          H -> return ()

deriving via Marshal.EquivStorable H instance BG.Storable H

deriving via Struct.IsStructViaReadRaw H instance Struct.IsStruct H

{-| __C declaration:__ @struct typedef3@

    __defined at:__ @types\/structs\/tagged_vs_untagged.h 44:9@

    __exported by:__ @types\/structs\/tagged_vs_untagged.h@
-}
data Typedef3 = Typedef3
  {}
  deriving stock (Eq, BG.Generic, Show)

instance Marshal.StaticSize Typedef3 where

  staticSizeOf = \_ -> (0 :: Int)

  staticAlignment = \_ -> (1 :: Int)

instance Marshal.ReadRaw Typedef3 where

  readRaw = \ptr0 -> pure Typedef3

instance Marshal.WriteRaw Typedef3 where

  writeRaw =
    \ptr0 ->
      \s1 ->
        case s1 of
          Typedef3 -> return ()

deriving via Marshal.EquivStorable Typedef3 instance BG.Storable Typedef3

deriving via Struct.IsStructViaReadRaw Typedef3 instance Struct.IsStruct Typedef3
