...
genTestsHs ::
  ...
  -> [TestRecord]
  -> IO ()
genTestsHs hsTestPath moduleName cTestHeaderPath lineLength testRecords =
  ...
  where
    ...
    hsTestModuleFfiFunctions =
      concatMap (getFfiFunctions cTestHeaderFilename cFunPrefix) testRecordsC

    hsTestModuleOrphanInstances :: [OrphanInstance]
    hsTestModuleOrphanInstances = concatMap getOrphanInstances testRecords

    hsTestModuleTypeTests :: [TypeTest]
    hsTestModuleTypeTests =
      [ TypeTest
          (fst $ getNameAndSpelling testRecord)
          (isCReferenceable testRecord)
      | testRecord <- testRecords
      ]

    hsTestModuleTestsFun :: TestsFun
    hsTestModuleTestsFun = TestsFun moduleName $
      fst . getNameAndSpelling <$> testRecords

getFfiFunctions :: IncludeFile -> CFunPrefix -> TestRecord -> [FfiFunction]
getFfiFunctions includeFile cFunPrefix = \case
    TestStruct{..}
      | Set.member SHs.Storable_Storable testStructInstances ->
          let name = either SHs.newtypeName SHs.dataType testStructDecl
          in  [ FfiSizeof   includeFile cFunPrefix name
              , FfiAlignof  includeFile cFunPrefix name
              , FfiGenSeqHs includeFile cFunPrefix name
              , FfiGenSeqC  includeFile cFunPrefix name
              , FfiPreturb  includeFile cFunPrefix name
              ]
      | otherwise -> []
    TestEnum{} -> [] -- TODO
    TestTypedef{} -> [] -- TODO

getOrphanInstances :: TestRecord -> [OrphanInstance]
getOrphanInstances = \case
    TestStruct{..} ->
      let typeName = either SHs.newtypeName SHs.dataType testStructDecl
          cnstName = either SHs.newtypeCon SHs.dataCon testStructDecl
          fieldNames =
            either
              (pure . SHs.fieldName . SHs.newtypeField)
              (map SHs.fieldName . SHs.dataFields)
              testStructDecl
      in  [ ArbitraryInstance     typeName cnstName fieldNames
...
data TypeTest = TypeTest (HsName NsTypeConstr) Bool
...
prettyFfiSizeof inc' pfx' name' =
    hang
      ( hcat
          [ "foreign import capi unsafe \"", inc, " "
          , pfx, "_sizeof_", name, "\""
          ]
      )
      2
      (hcat ["sizeof", name, " :: IO FC.CSize"])
  where
    inc, pfx, name :: CtxDoc
    inc  = string inc'
    pfx  = string pfx'
...
instance Pretty TypeTest where
  pretty (TypeTest name isC) = prettyTypeTest name isC

prettyTypeTest :: HsName NsTypeConstr -> Bool -> CtxDoc
prettyTypeTest name' isC =
    hang
      ...
      ( vlist '[' ']' $ catMaybes
          ...
          , justWhen isC $
              hang "THU.testCase GenSeq.nameHsGenSeq1SameSemanticsCGenSeq1 $" 2 $
                hcat
                  ["GenSeq.assertHsGenSeq1SameSemanticsCGenSeq1 genSeqC", name]
          , justWhen isC $
...
prettyTestsFun moduleName' typeNames' =
    hang
      ( "tests :: Tasty.TestTree"
          $$ hcat ["tests = Tasty.testGroup \"", moduleName, "\""]
      )
      4
      ( vlist '[' ']'
          [ "test" >< typeName
  ...
  where
    ...
    typeNames :: [CtxDoc]
    typeNames = prettyHsName <$> typeNames'
