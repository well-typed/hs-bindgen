...
data DataTyCon n where
  ...
  IntyTyCon   :: !PrimIntType -> DataTyCon Z
...
inferTop funNm argsList body = Vec.reifyList argsList $ \ args -> do
  ...
  return ( macroTy', simplifyCts cts' )
...
pattern Inty :: PrimIntType -> Type Ty
pattern Inty inty = TyConAppTy (DataTyCon (IntyTyCon inty)) VNil
...
-- satisfied.
newtype Models = Models [ Subst ( First TyVar ) ]

instance Semigroup Models where
  Models ms1 <> Models ms2 =
    Models $
      mapMaybe ( uncurry combineSubsts ) $
        [ ( m1, m2 ) | m1 <- ms1 , m2 <- ms2 ]
...
combineSubsts :: forall tv. Semigroup tv => Subst tv -> Subst tv -> Maybe ( Subst tv )
...
e.g.
  ...
  instance forall primIntTy. Num ( Inty primIntTy )
  instance Eq Bool
...
-- See Note [Class instances and DataTyConTag].
classDefaults :: ClassTyCon n -> Map ( Vec n DataTyConTag ) ( Vec n ( Type Ty ) )
classDefaults = Map.fromList . \case
...
classInstances = Map.keys . classDefaults
...
-- e.g. does @cls1 a1 a2@ imply @cls1 a1 a2@
...
tcMacro tyEnv macroNm args body = do
  ...
  let
    ...
    ( ( ty, ( ctsOrigs, mbIncons ) ), mbErrs ) = runTcM tyEnv ( inferTop macroNm args body )
  ...
  let
    ...
    Models mbModels = foldMap ( defaultCt ( tyVarSet quantTvs ) . fst ) ctsOrigs
  case NE.nonEmpty mbModels of
    Nothing -> do
      Left ( TcInconsistentConstraints ctsOrigs )
    Just ( defaultSubst NE.:| _otherSubsts ) -> do
      let
        -- Step 4: re-simplify constraints after defaulting.
        -- NB: no need to go around in a loop for such simple constraints.
        ( ctsOrigsAfterDefaulting, mbIncons2 ) =
          simplifyCts ( map ( first ( applySubst defaultSubst ) ) ctsOrigs )
      ...
      Vec.reifyList quantTvs $ \ ( quantTvsVec :: Vec n TyVar ) -> do
        Right $ QuantTy @n $ \ tys ->
          ...
          in QuantTyBody
              { quantTyQuant = map ( applySubst quantSubst ) ( fmap fst ctsOrigsAfterDefaulting )
...
defaultCt quantTvs ( TyConAppTy ( ClassTyCon cls ) ( args :: Vec n ( Type ty ) ) ) =
  Models $ Map.elems $ Map.mapMaybeWithKey ( defaultingSubst args ) ( classDefaults cls )
  where
    defaultingSubst :: Vec n ( Type Ty ) -> Vec n DataTyConTag -> Vec n ( Type Ty ) -> Maybe ( Subst ( First TyVar ) )
