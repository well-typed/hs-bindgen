...
deriving via SimpleEnum DataTyCon
  instance TestEquality DataTyCon
...
deriving via SimpleEnum ClassTyCon
  instance TestEquality ClassTyCon
...
instance Show k => Monoid ( Subst k ) where
...
pprCouldNotUnifyReason = \case
  ...
  OccursCheck tv ->
    "of an occurs-check in the variable '" <> tv <> "'"
  RigidSkolem tv ->
    "'" <> tv <> "' is a rigid skolem variable"
