...
resolveMacro declIds (Macro.Unresolved macro) = do
    resolvedMacro <- CExpr.annotateMacro resolve macro
    pure Macro.Resolved{
        macro = resolvedMacro
  ...
  where
    ...
    resolve name _ = case name of
      ...
      CExpr.NameTagged nm tag ->
        resolveTagged nm.getIdentifier (convertTagKind tag)
