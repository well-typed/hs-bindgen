...
parseMacro cStd macro =
  ...
  where
    ...
    -- 'CExpr.Macro' has no representation for a variadic parameter list, in
    -- either spelling; without this check @__VA_ARGS__@, or the GNU name
    -- standing for it, would silently parse as a free variable.
