...
data Options = Options {
    ...
    , optComments       :: Bool
    , optExtents        :: Bool
    , optFile           :: CHeaderRelPath
    , optIncludePath    :: [CIncludePathDir]
    , optKind           :: Bool
    , optSameFile       :: Bool
    , optSysIncludePath :: [CIncludePathDir]
    , optType           :: Bool
    }
...
clangAstDump opts@Options{..} = do
    putStrLn $ "## `" ++ getCHeaderRelPath optFile ++ "`"
    putStrLn ""

    sysIncludePath <- either fail pure
      =<< resolveCIncludeAbsPathDirs optSysIncludePath
    includePath <- either fail pure
      =<< resolveCIncludeAbsPathDirs optIncludePath
    absPath <- either fail pure
      =<< resolveHeader (includePath ++ sysIncludePath) optFile
    let fp = getCHeaderAbsPath absPath

    let clangArgs = defaultClangArgs {
            clangSystemIncludePathDirs = optSysIncludePath
          , clangIncludePathDirs       = optIncludePath
          }

    index <- clang_createIndex DontDisplayDiagnostics
    unit <- clang_parseTranslationUnit index fp clangArgs clangOpts
    ...
    void . HighLevel.clang_visitChildren rootCursor $
      \cursor -> do
        ...
        case loc of
          (file, _, _)
            | optSameFile && file /= T.pack fp -> pure $ Continue Nothing
  ...
  where
    ...
    clangOpts :: BitfieldEnum CXTranslationUnit_Flags
    clangOpts = bitfieldEnum [
...
main = clangAstDump . uncurry applyAll =<< OA.execParser pinfo
  where
    ...
    parseOptions = do
      ...
      optSysIncludePath <- sysIncludePathOption
      optIncludePath    <- includePathOption
      optFile           <- fileArgument
      pure (optAll, Options{..})

    sysIncludePathOption :: OA.Parser [CIncludePathDir]
    sysIncludePathOption = OA.many . OA.strOption $ mconcat
      [ OA.long "system-include-path"
      , OA.metavar "DIR"
      , OA.help "System include search path directory"
      ]

    includePathOption :: OA.Parser [CIncludePathDir]
    includePathOption = OA.many . OA.strOption $ mconcat
      [ OA.short 'I'
      , OA.long "include-path"
      , OA.metavar "DIR"
      , OA.help "Non-system include search path directory"
      ]

    fileArgument :: OA.Parser CHeaderRelPath
    fileArgument = OA.argument (OA.eitherReader mkCHeaderRelPath) $ mconcat
      [ OA.metavar "FILE"
