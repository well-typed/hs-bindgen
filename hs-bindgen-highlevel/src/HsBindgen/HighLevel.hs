...
--
-- A binding is a /spec/: one position per C argument, chained with @($)@ and closed
-- by a result converter, then run against the raw @foreign import@ with
-- 'toHighLevel'.
--
-- > hsStrncmp :: String -> ByteString -> IO Int
-- > hsStrncmp = toHighLevel ( input  withCStringIn        -- 1 C argument
-- >                         $ input2 useAsByteStringLenIn -- 2 C arguments
-- >                         $ resultPure fromIntegral
-- >                         ) c_strncmp
--
-- It reads top to bottom like an annotated copy of the C prototype, and each position
...
--   * 'scratch', 'scratchArray' and 'fixed' fill a C argument the wrapper never
--     exposes;
--   * a closer ('resultPure', 'resultIO' and friends) turns the C return value, plus
--     any outputs, into the wrapper's result.
--
-- The closer's function is the /assembler/: it receives every 'output' value in spec
-- order, then the C return value, and decides the wrapper's result. So
...
-- fill the ordinary positions from the wrapper's type signature.
--
...
import HsBindgen.HighLevel.Internal.Spec (ApplyOutputs (..), Assembler,
                                          Outputs (..), ToHighLevel (..),
                                          toHighLevel)
...
-- offer the matching marshaller as a valid hole fit), and every position after it
...
-- | Add a scratch position: a bracket supplying one C argument the call writes
-- into and the caller never sees. It contributes nothing to the wrapper type, neither
-- an argument nor a result component.
--
...
-- a flags word, a NULL callback, a context handle, a buffer allocated by the caller.
-- Where 'scratch' allocates something for the duration of the call, 'fixed' pins a
-- value you already have:
--
-- > allocaBytes errbufSize $ \errbuf ->
-- >   toHighLevel ( output peekDeviceNames  -- pcap_if_t **
-- >               $ fixed errbuf            -- char *: ours, kept out of the wrapper type
-- >               $ resultIO (check errbuf)
-- >               ) pcap_findalldevs
--
...
-- order, then the C return value, and returns the wrapper's result. Its type is
-- spelled out by 'Assembler', so GHC reports it concretely in a hole.
--
...
-- | Drop a t'MarshalStruct' into an 'input' position: the wrapper gains one @hi@ argument
...
-- | Drop a t'MarshalStruct' into a @const T *@ argument position, the @const@ form of
...
-- | Drop an t'UnmarshalStruct' into an 'output' position: allocate a @'Ptr' struct@ for
...
--
-- > encrypt = toHighLevel ( output ciphertextOut
...
-- how to read them is how you find the position that went wrong.
--
...
--   thing; each position removes the C arguments it fills. A primed @lo'@ in a combinator's type always
--   means \"what is left after this position\".
--
...
--   sees the finished list and asks for an 'Assembler' over it.
--
-- Everything else names a single value at a single position: @hs@ a Haskell one, @c@
-- a C one.
--
...
-- >        -> ToHighLevel os (c -> lo') (hs -> hi) -- this position and everything below
--
-- Going down the chain @lo@ loses a C argument and @hi@ loses a wrapper argument, in
...
--
-- @lo@ unifies with the C function's actual type, @hi@ with the high-level type signature.
-- GHC has nothing left to guess, which is why holes come back concrete and why @auto@ can
-- work at all.
--
-- Two consequences matter when an error does appear. A spec is checked as a whole, so
-- one wrong position often surfaces as a mismatch reported at a neighbouring one:
-- compare the @lo@ in the message against the C prototype to find where the two
-- stopped lining up. And a wrapper with /no/ type signature leaves @hi@ open, so
...
--
-- This spec builds the same function:
--
-- > hsFrexp = toHighLevel (input defaultIn $ output defaultOut $ autoResult) c_frexp
--
-- The difference is that the hand-written version sees the whole call at once, whereas
-- each combinator sees only its own position: 'output' cannot see how many arguments
-- precede it, or what follows it.
--
-- == When each part runs
--
...
-- the arguments of the positions above already applied.
--
...
-- once the resulting @IO@ action finishes. Nesting one 'threadIn' per position opens
...
-- >        -> ToHighLevel os        (c -> lo') hi
--
-- One C argument is consumed, so @c -> lo'@ becomes @lo'@. No wrapper argument is
...
-- could be done. The suggested way to do it is to write the wrapper's type signature
-- first, then fill the spec one position at a time, holding a hole @_@ at the position
-- you are working on and __stubbing the tail with @auto@__.
--
...
--
-- > hsParseInt = toHighLevel ( input _
-- >                          $ output peekIntOut
-- >                          $ auto
-- >                          ) parse_int
...
--
-- The output's Haskell type is concrete because the wrapper's result type
...
-- @c_strncmp :: Ptr CChar -> Ptr CChar -> CSize -> IO CInt@ and a wrapper returning
-- 'Bool', a hole in the closer comes back fully applied:
--
-- > hs1 :: Ptr CChar -> Ptr CChar -> CSize -> IO Bool
-- > hs1 = toHighLevel (autoInputs $ resultPure _) c_strncmp
...
--
-- > hs2 = toHighLevel (autoInputs _) c_strncmp
...
--
-- > hs4 = toHighLevel (autoInputs _) c_frexp
-- >   -- Found hole: _ :: ToHighLevel '[] (Ptr CInt -> IO CDouble) (IO (Int, Double))
-- >
-- > hs4 = toHighLevel (autoInputs $ output _ $ autoResult) c_frexp
-- >   -- Found hole: _ :: Unmarshaller (Ptr CInt) Int
--
-- The @Ptr CInt ->@ still sitting in the first message is the position you have not
...
-- result, nothing else.
--
-- @auto cannot assemble this result@ means @auto@ reached the closing position but
-- the outputs it collected and the C return do not add up to the result type in the
-- signature. The message lists all three. Either give the wrapper that result type,
...
--
-- A wrong assembler is reported against 'Assembler', which GHC prints unreduced:
--
-- > Expected: Assembler '[Int, Int] (CInt -> Pair)
-- >   Actual: Int -> Int -> Pair
--
...
-- > hsFillInner = toHighLevel
-- >   (output (asOutput innerOut) $ resultPure (\i c -> (i, fromIntegral c))) c_fillInner
-- >
