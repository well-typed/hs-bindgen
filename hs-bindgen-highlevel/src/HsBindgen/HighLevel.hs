...
--
-- It stays a separate final pass rather than folding into the closer because the
-- trailing @()@ earns its keep while the spec is built: it is what lets an output's
-- value type be recovered from the declared result (the inverse direction of
-- 'HsBindgen.HighLevel.Internal.Threading.ThreadOut'). Drop it any earlier
-- and that inference breaks, so it can only go once the spec is complete.
--
