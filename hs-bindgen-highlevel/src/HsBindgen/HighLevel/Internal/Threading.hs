-- | Holding a bracket open across a curried wrapper's remaining arguments.
--
...
--
-- Every position in a spec opens one: 'Foreign.C.String.withCString' for a string
...
-- until the C call happens, and the C call happens only once the wrapper has been
-- given /all/ of its arguments. But the wrapper is curried, so the first argument
...
-- a bracket supplying an @a@, and a way to turn that @a@ into the wrapper @hi@, build
-- @hi@ with the bracket open around the point where @hi@ finally reaches 'IO'/.
--
...
-- 'threadIn's, one per position, opens them outermost-first and closes them in the
-- reverse order, so a throw anywhere unwinds all of them.
--
-- The class is indexed on the wrapper type alone. What the bracket supplies is
