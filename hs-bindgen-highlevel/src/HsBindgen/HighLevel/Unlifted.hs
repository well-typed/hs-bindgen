...
-- > allocW k = snd <$> preallocate @(OnHaskellHeap tag) k
--
...
-- compile, because @(.)@ cannot take an unlifted argument. The same restriction is why
-- @allocW@ is written applied rather than point-free.
--
...
-- never bound by representation-polymorphic code, so wrapper arguments and further
