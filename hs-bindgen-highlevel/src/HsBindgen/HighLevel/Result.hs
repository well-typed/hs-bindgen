...
-- | The result type of 'assertPure': the wrapper's own type with @IO@ stripped off the
-- result. A wrapper @a -> b -> IO c@ purifies to @a -> b -> c@.
...
-- about the returned value is guaranteed pure: 'purify' produces it with
-- 'unsafePerformIO', so correctness rests entirely on the caller's assertion that the
-- call is deterministic (see 'assertPure').
class Purifiable hi where
  -- | Usually invoked through 'assertPure'.
  purify :: hi -> Purify hi
