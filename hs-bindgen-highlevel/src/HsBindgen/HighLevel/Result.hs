...
-- > encrypt = toHighLevelPure spec crypto_secretbox_easy
--
...
{-------------------------------------------------------------------------------
  ...
  A spec produces a wrapper ending in IO; 'toHighLevelPure' produces the pure
  ...
  wrapper type has to be computed *from* it.
-------------------------------------------------------------------------------}

-- | The wrapper type a pure signature stands for: every argument kept, the result
-- moved into 'IO'.
--
...
--
-- The first equation wins on an arrow, so a wrapper whose /result/ is itself a
...
-- takes an argument.
--
-- Indexed on the signature @hs@ rather than on the wrapper type, so that the walk is
-- driven by the type the binding was given. The wrapper type it consumes is
...
-- > hsHypot = toHighLevelPure auto c_hypot
toHighLevelPure ::
     forall hs lo. Purifiable hs
  => ToHighLevel '[] lo (Unpurify hs)
  -> lo
  -> hs
toHighLevelPure spec = purifyAt @(IsFunction hs) . toHighLevel spec
