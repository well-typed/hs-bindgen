...
--     (@Int@ ↔ @CInt@, @Word@ ↔ @CUInt@, @Bool@ ↔ @CBool@, @CSize@ -> @Int@ (result
--     only), and so on), so a wrapper written in idiomatic Haskell needs no marshaller.
...
--     @intptr@, @sig_atomic@, and Unicode typedefs, and the fixed-width types).
...
-- | The canonical output marshaller for a Haskell type @hs@, reading a C
-- out-parameter into a Haskell value (converting the idiomatic scalars, otherwise
-- reading it back unchanged). Buffer-sized outputs (strings, arrays) have no canonical
...
-- return closes to 'Int' (@IO Int@) or stays raw (@IO CInt@). The result type is read
-- from the wrapper signature rather than inferred, so annotate it.
--
