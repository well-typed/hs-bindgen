...
hsStrlenOne = toHighLevel (input withCStringIn $ resultPure fromIntegral) c_strlen
...
hsReportLen = toHighLevel
    (input2 constByteStringLenIn $ resultPure fromIntegral) reportLen

hsEchoLen :: ByteString -> IO ByteString
hsEchoLen = toHighLevel (input2 unsafeByteStringLenIn $ resultPure id) echoBytesLen
