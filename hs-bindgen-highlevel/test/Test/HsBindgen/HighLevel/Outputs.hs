...
hsUnmarshalOutWith = toHighLevel
    (output (unmarshalOutWith alloca readInt) $ resultPure (\v c -> (v, fromIntegral c))) writeInt
  where
...
hsWriteError = toHighLevel
    (output (peekCStringOut 32) $ resultPure (\s c -> (s, fromIntegral c))) writeErrorString

hsBytesOut :: IO (ByteString, Int)
hsBytesOut = toHighLevel
    (output (byteStringOut 3) $ resultPure (\b c -> (b, fromIntegral c))) writeThreeBytes
