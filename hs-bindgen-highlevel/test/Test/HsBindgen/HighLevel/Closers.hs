...
hsDiscard :: CInt -> IO ()
hsDiscard = toHighLevel (input (scalar id) $ discardResult) returnsStatus

pureAdd :: Int -> Int -> Int
pureAdd = toHighLevelPure
...
-- pure one, so the wrapper type has to be computed from it rather than inferred out
-- of the spec.
pureAddAuto :: Int -> Int -> Int
pureAddAuto = toHighLevelPure auto cAdd

-- | The base case of the walk, with no arguments to recurse under.
pureStatus :: Int
pureStatus = toHighLevelPure auto (returnsStatus 0)
...
hsOneOutOnly :: IO Int
hsOneOutOnly = toHighLevel (output peekIntOut $ resultPure (\v _ -> v)) oneOut

hsTwoOutOnly :: IO (Int, Int)
hsTwoOutOnly = toHighLevel
    ( output peekIntOut
    $ output peekIntOut
