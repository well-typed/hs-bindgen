...
module HsBindgen.Runtime.BitfieldPtr (
    BitfieldPtr(BitfieldPtr, ptr, offset, width)
  , peek
  , poke
  ) where
...
pattern BitfieldPtr ptr o w <- UnsafeBitfieldPtr ptr o w where
  BitfieldPtr ptr o w = UnsafeBitfieldPtr (ptr `plusPtr` nBytes) o' w
    where (nBytes, o') = o `quotRem` 8

{-# INLINE peek #-}
-- | Read from a bit-field using a pointer
peek :: Bitfield a => BitfieldPtr a -> IO a
peek (BitfieldPtr ptr o w) = Bitfield.peekBitOffWidth ptr o w

{-# INLINE poke #-}
-- | Write to a bit-field using a pointer
poke :: Bitfield a => BitfieldPtr a -> a -> IO ()
poke (BitfieldPtr ptr o w) val = Bitfield.pokeBitOffWidth ptr o w val
