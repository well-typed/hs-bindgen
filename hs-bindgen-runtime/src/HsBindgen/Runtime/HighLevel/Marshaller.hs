...
-- "HsBindgen.Runtime.HighLevel.Marshaller.Utils"; to lift a low-level /function/,
-- see "HsBindgen.Runtime.HighLevel".
--
module HsBindgen.Runtime.HighLevel.Marshaller (
    -- * The marshaller type
    Marshal (..)
  , at
  , scalar
...
-- @f@ then @g@. The function side uses 'HsBindgen.Runtime.HighLevel.input' (or
...
--
-- @at@ checks the field's C type and its order (through the constructor arrow),
-- not which selector was projected. Two fields of the same Haskell type can be
-- swapped without a type error, so the selector is the caller's responsibility.
...
-- 'HsBindgen.Runtime.HighLevel.asArgument'.
--
...
--
newtype UnmarshalStruct lo hi = UnmarshalStruct { runUnmarshalStruct :: lo -> IO hi }
