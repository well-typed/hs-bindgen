...
mkDerivation {
  ...
  testHaskellDepends = [
    async base c-expr-dsl c-expr-runtime containers data-default
    debruijn directory filepath hs-bindgen-runtime libclang-bindings
    mtl optics optics-core parsec process syb tasty tasty-hunit
    tasty-quickcheck template-haskell temporary text utf8-string vector
  ];
  doHaddock = false;
