...
# Library mode walks the include graph of rpmlib.h (which transitively includes
# all 13 RPM public headers), assigns each sub-header its own Haskell module,
# and chains binding specs automatically in dependency order.
cabal run hs-bindgen-cli -- preprocess \
    -I "$INCLUDE_DIR" \
    --library "$INCLUDE_DIR/rpm" \
