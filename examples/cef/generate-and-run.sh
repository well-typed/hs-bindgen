...
# All 79 capi headers in one call. Library mode walks the include graph of
# cef_app_capi.h (the top-level header that transitively includes every other
# capi header), assigns each sub-header its own Haskell module, and chains
# binding specs automatically in dependency order.
cabal run hs-bindgen-cli -- preprocess \
    -I "$INCLUDE_DIR" \
    --library "$INCLUDE_DIR" \
    --hash-define CEF_API_VERSION 14500 \
