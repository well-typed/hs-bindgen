...
│   └── Makefile
├── hs-project-preprocess/     # Preprocess-mode example (Phase 3)
│   ├── cross-compilation-preprocess.cabal
│   ├── cabal.project          # only hs-bindgen-runtime
│   ├── app/
│   │   ├── Main.hs            # uses pre-generated bindings
│   │   ├── ArchSizes.hsc
│   │   └── PrintStructs.hs
│   └── src-aarch64/           # bindings generated by hs-bindgen-cli
└── hs-project-th/             # TH-mode example (Phase 4)
    ├── cross-compilation-th.cabal
    ├── cabal.project          # full hs-bindgen + transitive deps
    └── app/
        ├── MainTH.hs          # generates bindings inline via withHsBindgen
        ├── ArchSizes.hsc      # (duplicated from preprocess project)
        └── PrintStructs.hs    # (duplicated from preprocess project)
```

The two `hs-project-*` directories are deliberately kept fully separate:
the preprocess project only depends on `hs-bindgen-runtime`, while the TH
project pulls in the full `hs-bindgen` package and its transitive
libclang/zlib deps. Each has its own `cabal.project.local`, generated by
`generate-and-run.sh` with only the lib/include paths that mode actually
needs.
...
hs-bindgen-cli preprocess \
  ...
  --hs-output-dir hs-project-preprocess/src-aarch64 \
  c-src/arch_types.h
```
