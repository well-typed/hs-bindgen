...
import HsBindgen.Runtime.HighLevel (fixed, input, input2, output, toHighLevel)
...
import LibGit2.Marshal (asArgumentC, bufferIn, handleIn, handleInC, newHandle,
                        nullConst, oidInC, oidOut, textIn)
...
repositoryInit path =
    fst <$> newHandle RF.git_repository_free
...
treebuilderNew repo =
    fst <$> newHandle TF.git_treebuilder_free
              ( input handleIn
...
treeLookup repo oid =
    fst <$> newHandle TF.git_tree_free (input handleIn . input oidInC)
