...
# Botan C++ library, pinned to a release tag so the example is reproducible. We
...
(
    ...
    if [ ! -d "$SCRIPT_DIR/botan/.git" ]; then
        ...
        # demand) while still allowing checkout of the pinned tag; a shallow
        # clone could not check out an arbitrary older tag.
        git clone --filter=blob:none "$BOTAN_REPO" "$SCRIPT_DIR/botan"
    fi
    git -C "$SCRIPT_DIR/botan" checkout --quiet "$BOTAN_TAG"
